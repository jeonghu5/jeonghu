...
BOOL CTimering2Dlg::OnInitDialog()
{
	...
	m_Break1 = CRect(0,0 ,50 ,20 );
	m_Break2 = CRect(100,0 ,150 ,20 );
	m_Break3 = CRect(200, 0, 250, 20);
	m_Break4 = CRect(300, 0,350 ,20 );
	m_Break5 = CRect(400,0 , 450,20 );
...
void CTimering2Dlg::OnTimer(UINT_PTR nIDEvent)
{
	...
	CBrush br;
	br.CreateSolidBrush(RGB(0, 0, 100));
	dc.SelectObject(&br);
	dc.Rectangle(m_Break1);
	dc.Rectangle(m_Break2);
	dc.Rectangle(m_Break3);
	dc.Rectangle(m_Break4);
	dc.Rectangle(m_Break5);
	...
	CRect hi;
	if (IntersectRect(&hi, &m_Ball, &m_Break1))
	{
		m_Break1.DeflateRect(25, 0);
		b = -b;
		m_Ball.OffsetRect(0, 20);
	}
	if (IntersectRect(&hi, &m_Ball, &m_Break2))
	{
		m_Break2.DeflateRect(25, 0);
		b = -b;
		m_Ball.OffsetRect(0, 20);
	}
	if (IntersectRect(&hi, &m_Ball, &m_Break3))
	{
		m_Break3.DeflateRect(25, 0);
		b = -b;
		m_Ball.OffsetRect(0, 20);
	}
	if (IntersectRect(&hi, &m_Ball, &m_Break4))
	{
		m_Break4.DeflateRect(25, 0);
		b = -b;
		m_Ball.OffsetRect(0, 20);
	}
	if (IntersectRect(&hi, &m_Ball, &m_Break5))
	{
		m_Break5 .DeflateRect(25, 0);
		b = -b;
		m_Ball.OffsetRect(0, 20);
	}
	...
	if (IntersectRect(&hi, &m_Ball, &m_Player))
	{
		...
		{
			f = 10;
			m_Ball.OffsetRect(f, b);m_Ball.OffsetRect(f, b);
		}
		else if (c > d)
		{
			f = -10;
